...
def is_clean_noun(word):
    ...
    if tag in ('NNP', 'NNPS'):
        noun_priority_map[word] = 1  # Priority 1 for Proper nouns
        return True
    elif tag in ('NN', 'NNS'):
        noun_priority_map[word] = 2  # Priority 2 for Common nouns
        return True
...
def get_top_n_tfidf_words(summarizer, n=10):
    ...
    sorted_words = sorted(
        all_word_scores.items(),
        key=lambda item: (noun_priority_map.get(item[0], 3), -item[1])
    )
    
    return dict(sorted_words[:n])

def Extractive_Summarizer(input_text: str, ratio: float, selectedOptionValue:str):
